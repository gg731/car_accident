    ...
    public void reg(Class cl) {
        ...
        for (Class arg : c.getParameterTypes()) {
            if (!els.containsKey(arg.getCanonicalName())) {
                throw new IllegalStateException("Object doesnt found in context");
            }
            args.add(els.get(arg.getCanonicalName()));
        }
        try {
            els.put(cl.getCanonicalName(), c.newInstance(args.toArray()));
        } catch (Exception e) {
            throw new IllegalStateException("Count create an instance");
        }
    }
